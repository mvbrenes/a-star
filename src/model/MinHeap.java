...
{
	private ArrayList<Comparable<Node>> elements;

	/**
	 * Constructs an empty heap.
	 */
	public MinHeap() {
		elements = new ArrayList<Comparable<Node>>();
		elements.add(null);
	}
	 ...
	 *            the element to add
	 */
	public void add(Comparable<Node> newElement) {
		// Add a new leaf
		elements.add(null);
		int index = elements.size() - 1;

		// Demote parents that are larger than the new element
		while (index > 1 && getParent(index).compareTo((Node) newElement) > 0) {
	 ...
	 * @return the minimum element
	 */
	public Comparable<Node> remove() {
		Comparable<Node> minimum = elements.get(1);

		// Remove last element
		int lastIndex = elements.size() - 1;
		Comparable<Node> last = elements.remove(lastIndex);
	...
	private void fixHeap() {
		Comparable<Node> root = elements.get(1);
	 ...
	 */
	private Comparable<Node> getLeftChild(int index) {
	 ...
	 */
	private Comparable<Node> getRightChild(int index) {
	 ...
	 */
	private Comparable<Node> getParent(int index) {
